...
class VMClotoide : ViewModel() {
    ...
    fun getListCalculates(): List<CalculateModel> = listOf(
        ...
        CalculateModel("0e", "${calcThetaE().toDMS()}"),
        CalculateModel("Ac", "${calcDeltaC().toDMS()}"),
        CalculateModel("Gc", "${calcGc().toDMS()}"),
        CalculateModel("lcc", "${calcLcc().customFormat()} m"),
        CalculateModel("0r", "${calcSigmaRad().customFormat()}"),
        ...
        CalculateModel("E", "${calcPhiE().toDMS()}"),
    ...
    private fun table1(ini: Double, fin: Double) {
        ...
        val newTe = ini.customFormat().roundToInt().roundToNearestTen()
    ...
    private fun deflexionTable() {
        ...
        val newPc = calcProgEC().roundToInt().roundToNearestTen()
        ...
        rows2.add(
            RowDeflexionModel(
                ...
                table.calcDeflexionUnitaria(disPoints, rc.toDouble()),
                table.calcDeflexionUnitaria(disPoints, rc.toDouble()),
                table.calcDeflexionUnitaria(disPoints, rc.toDouble()),
            )
        )
        ...
        rows2.add(
            RowDeflexionModel(
                ...
                table.calcDeflexionUnitaria(calcProgCE() - lastRow.progresiva, rc.toDouble()),
                lastRow.deflexAcumulada + table.calcDeflexionUnitaria(
    ...
    private fun table3() {
        ...
        val newTe = calcProgET().customFormat().roundToInt().roundToFarthestTen()
    ...
    private fun table6() {
        ...
        rows6.add(
            RowCoordenadas(
                ...
                deltaN = 0.0,
                deltaE = 0.0,
                n = n + (calcTc() * cos(az2.toDegree().degreeToRadian())),
                e = e + (calcTc() * sin(az2.toDegree().degreeToRadian())),
            )
        )


        var newPc = calcProgEC().roundToInt().roundToNearestTen()
        ...
        rows6.add(
            RowCoordenadas(
                newPc.toDouble(), disPoints, defUnitaria, defAcumulada, azimut, disPoints, deltaN, deltaE,
                valueN, valueE
            )
        )
...
//
    ...
    private fun table7() {
        ...
        val dProg = calcProgCE().customFormat()

        var newAProg = calcProgET().customFormat().roundToInt().roundToFarthestTen()
        var newL = (newAProg - calcProgCE()).customFormat()
        var theta = calcThetaE(newL)
        var thetaRad = calcThetaRad(newL)
        var x = calcXc(newL, thetaRad)
        var y = calcYc(newL, thetaRad)
